...
class WaterRippleData: ObservableObject {
    @Published var props: NSMutableDictionary = [:]
    //  var toggle: ((_ showing: Bool) -> Void)?
}

struct WaterRippleDemo: View {
    @ObservedObject var data = WaterRippleData()
    ...
    var body: some View {
        let scaleProp = data.props.value(forKey: "scale") as! Double
        let sharpnessProp = data.props.value(forKey: "sharpness") as! Double
        let spreadProp = data.props.value(forKey: "spread") as! Double
        WaterEffect(scale: scaleProp, sharpness: sharpnessProp, spread: spreadProp) {
                ZStack(alignment: .top) {
                    ...
                    VStack(alignment: .center) {
                        ...
                        Text("Cyril Zakka is Amazing")
                        Text("NativeScript driving with Storybook")
