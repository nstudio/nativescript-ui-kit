...
import { CoachMarks, ICoachMarkOptions, CoachMark } from '@nstudio/nativescript-coachmarks';
const items = new Map();
export function onLoaded(args) {
  ...
  setTimeout(() => {
    const startWithOptions = () => {
      ...
      CoachMarks.start(getMarks(), options, this._coachMarks);
    };

    const getMarks = () => {
      const fb = items.get('fb');
      const google = items.get('google');
      const x = items.get('x');
...
export function itemLoaded(args) {
  items.set(args.object.id, args.object);
}
