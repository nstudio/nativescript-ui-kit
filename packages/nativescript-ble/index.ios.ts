...
class CBPeripheralDelegateImpl extends NSObject implements CBPeripheralDelegate {
    ...
    static initWithOwner(owner: WeakRef<Bluetooth>): CBPeripheralDelegateImpl {
        const delegate = <CBPeripheralDelegateImpl>CBPeripheralDelegateImpl.new();
        delegate._owner = owner;
        delegate.subDelegates = [];
        delegate.onNotifyCallbacks = {};
        if (Trace.isEnabled()) {
            CLog(CLogTypes.info, `CBPeripheralDelegateImpl.initWithOwner: ${owner.deref()}`);
        }
        return delegate;
    }
...
class CBCentralManagerDelegateImpl extends NSObject implements CBCentralManagerDelegate {
    ...
    static initWithOwner(owner: WeakRef<Bluetooth>): CBCentralManagerDelegateImpl {
        const delegate = <CBCentralManagerDelegateImpl>CBCentralManagerDelegateImpl.new();
        delegate._owner = owner;
        delegate.subDelegates = [];
        if (Trace.isEnabled()) {
            CLog(CLogTypes.info, `CBCentralManagerDelegateImpl.initWithOwner: ${delegate._owner}`);
        }
    ...
    public centralManagerDidConnectPeripheral(central: CBCentralManager, peripheral: CBPeripheral) {
        ...
        const owner: Bluetooth = this._owner && this._owner.deref();
        if (owner) {
            owner.onPeripheralConnected(peripheral);
        }
    ...
    public centralManagerDidDisconnectPeripheralError(central: CBCentralManager, peripheral: CBPeripheral, error?: NSError) {
        ...
        const owner: Bluetooth = this._owner && this._owner.deref();
        if (owner) {
            owner.onPeripheralDisconnected(peripheral);
        }
        this.subDelegates.forEach((d) => {
    ...
    public centralManagerDidDiscoverPeripheralAdvertisementDataRSSI(central: CBCentralManager, peripheral: CBPeripheral, advData: NSDictionary<string, any>, RSSI: number) {
        ...
        owner.addDiscoverPeripheral(peripheral);
...
export class Bluetooth extends BluetoothCommon {
    ...
    private _peripheralDelegates: { [key: string]: CBPeripheralDelegateImpl } = {};
    ...
    get centralDelegate() {
        if (!this._centralDelegate) {
            this._centralDelegate = CBCentralManagerDelegateImpl.initWithOwner(new WeakRef(this));
        }
    ...
    private prepareConnectedPeripheralDelegate(peripheral: CBPeripheral) {
        if (!peripheral?.delegate) {
            const UUID = NSUUIDToString(peripheral.identifier);
            this._peripheralDelegates[UUID] = CBPeripheralDelegateImpl.initWithOwner(new WeakRef(this));
            peripheral.delegate = this._peripheralDelegates[UUID];
        }
    }
    ...
    public onPeripheralDisconnected(peripheral: CBPeripheral) {
        if (peripheral) {
            ...
            if (peripheral.delegate) {
                ...
                delete this._peripheralDelegates[UUID];
            }
    ...
    public startScanning(args: StartScanningOptions) {
        ...
        return new Promise<void>((resolve, reject) => {
            try {
                ...
                if (this.scanningReferTimer && typeof this.scanningReferTimer.timer === 'number') {
                    clearTimeout(this.scanningReferTimer.timer);
                    if (this.scanningReferTimer.resolve) {
        ...
        return new Promise<void>((resolve, reject) => {
            try {
                ...
                if (this.scanningReferTimer) {
                    ...
                    if (typeof this.scanningReferTimer.timer === 'number') {
                        clearTimeout(this.scanningReferTimer.timer);
                    }
    ...
    public addDiscoverPeripheral(peripheral) {
        const UUID = NSUUIDToString(peripheral.identifier);
